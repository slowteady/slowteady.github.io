...
비동기를 사용하지 않는다면 시간이 오래 걸리는 일을 진행하는 동안 아무 동작도 하지 못하고 기다려야한다.  
비동기를 사용하면 시간이 오래 걸리는 일을 진행하도록 두고 그 다음 일을 처리하기 때문에 일을 병렬적으로 처리하여 처리 속도가 향상시킬 수 있다.
...
비동기로 동작하는 ajax의 일이 언제 끝나는지 모르기 때문에 일의 처리 순서를 보장 받을 수 없고, 에러가 출력된다.  

> 비동기의 문제점은 바로 일의 처리 순서를 보장 받을 수 없다는 것이다.

### 2. 콜백 함수

비동기 업무의 처리 순서를 보장 받기 위해 콜백 함수를 사용한다.
...
이와 같이 사용한다면 코드의 복잡도가 올라가기 때문에 유지보수가 어려워지고 가독성을 해친다.
...
이를 해결하기 위해 ES6에서 Promise 키워드가 등장했다.  
...
처리 상태라는 것을 의미하고 비동기 처리 로직이 완료되었다는 것이다.  
Settled 상태는 Fulfilled, Rejected 상태로 나뉜다.
...
위와 같이 resolve 함수를 호출하면 Fulfilled 상태가 된다.  
...
await 키워드를 promise를 리턴 값을 받을 함수 앞에 붙이고 async를 await 키워드를 사용하는 함수에 붙여서 사용한다.  
